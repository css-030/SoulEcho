...
### 2. 准备 API Key 与 Cookie

你可以直接在 SoulEcho 的设置页填写这些配置。对普通本地使用来说，`.env.local` 不是必需的。

| 配置项 | 从哪里获得 | 用来做什么 | 支持的功能 | 是否必需 |
| --- | --- | --- | --- | --- |
| OpenAI API Key | OpenAI Platform | 让 momo 使用 AI 模型理解和回应你的聊天内容 | AI 对话、情绪识别、音乐/疗愈建议生成、月度总结 | 推荐填写 |
| OpenWeather API Key | OpenWeather | 获取当前城市天气，把天气作为 momo 判断状态和疗愈音乐的上下文 | 天气感知聊天、按天气调整疗愈音乐氛围 | 可选 |
| YouTube API Key | Google Cloud 的 YouTube Data API v3 | 让 SoulEcho 进行真实 YouTube 音乐搜索 | YouTube 音乐推荐、日常歌单搜索 | 可选；不填会使用本地兜底曲库 |
| 网易云 API 地址 | 本地启动的 `NeteaseCloudMusicApi` 服务，默认 `http://localhost:3000` | 连接本地网易云 API，用于搜索歌曲、获取播放信息、检测 Cookie | 网易云音乐搜索、疗愈歌单、喜欢歌曲读取 | 使用网易云相关功能时需要 |
| 网易云 Cookie | 从你自己的网易云登录会话中获取，并只填在本地设置页 | 让本地网易云 API 以你的账号身份读取“我喜欢的音乐”等个人歌单 | 分析常听歌手、曲风和偏好，建立 momo 的音乐口味画像 | 想基于网易云收藏推荐时需要 |

请只在自己的本地环境中填写 Cookie，不要把 `.env.local`、API Key、Cookie 或 Token 提交到 GitHub，也不要分享给他人。

### 3. 可选：用 `.env.local` 预设默认值

如果你希望在开发时预设默认配置，或者希望清空浏览器数据后仍然保留一组默认值，可以复制 `.env.example` 为 `.env.local`：
...
2. 进入设置页，填写 OpenAI API Key，并按需要填写 OpenWeather、YouTube、网易云 API 地址与网易云 Cookie。网易云 Cookie 只用于在本地读取你的“我喜欢的音乐”等歌单，帮助 momo 分析你的听歌口味。
3. 回到聊天页，直接和 momo 对话。
